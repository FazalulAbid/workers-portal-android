...
class CategorySource(
    ...
    private val source: CategorySource.Source
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Category> {
        return try {
            ...
            val categories = when (source) {
                is Source.SearchCategories -> api.searchCategory(
                    page = nextPage,
                    pageSize = Constants.DEFAULT_PAGINATION_SIZE,
                    searchQuery = source.searchKey
                ).data ?: emptyList()

                else -> emptyList()
            }
            LoadResult.Page(
                data = categories.map { it.toCategory() } ?: emptyList(),
